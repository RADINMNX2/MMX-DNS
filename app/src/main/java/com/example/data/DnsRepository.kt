...
class DnsRepository(
    ...
    suspend fun ensureDefaultProfilesExist() {
        ...
        val defaultList = listOf(
            ...
            DnsProfile(name = "Electro DNS", primaryDns = "78.157.42.100", secondaryDns = "78.157.42.101", enableIpv6 = false, primaryIpv6 = "", secondaryIpv6 = "", isDefault = false, isCustom = false),
        ...
        for (def in defaultList) {
            val exists = existing.any { it.primaryDns == def.primaryDns || it.name.trim().equals(def.name.trim(), ignoreCase = true) }
            if (!exists) {
