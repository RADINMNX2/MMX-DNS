...
pub async fn resolve_via_doh3(
    ...
    let target_ip = if !bootstrap_ips.is_empty() {
        log::info!("DoH3 Client: Bootstrapping secure DNS domain '{}' directly to Anycast IP: '{}'", dns_ip, bootstrap_ips[0]);
    ...
    let peer_addr: SocketAddr = format!("{}:{}", target_ip, port)
        .parse()
        .map_err(|e| Doh3Error::EngineError(format!("Invalid DNS IP address ({}): {}", target_ip, e)))?;
    ...
    config.set_max_recv_udp_payload_size(1472);
    config.set_max_send_udp_payload_size(1472);
